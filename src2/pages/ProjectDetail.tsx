...
const ProjectDetail: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (id) {
            const project = allProjects.find(p => p.slug === id || (id && p.id === parseInt(id)));
            if (project) {
            ...
            setActiveProject(firstProject || null);
        }
    ...
    const handleCategoryChange = (category: string) => {
        ...
        const firstInCat = (projectsData.projects as Project[]).find(p => p.categories.includes(category));
        setActiveProject(firstInCat || null);
    };
    ...
    const filteredProjects = (projectsData.projects as Project[]).filter(p => 
        p.categories.includes(activeCategory)
    );

    const formatTimelineDate = (dateRange: string | { start?: string; end?: string } | undefined): string => {
