...
const mapCoordinates: any[] = contactConfig.mapCoordinates || [];

const checkIsCompanyOpen = (officeHours: OfficeHour[] | undefined): boolean | null => {
...
const Footer: React.FC<FooterProps> = ({ company = {} }) => {
  ...
  useEffect(() => {
    const checkOpen = () => {
      setIsOpen(checkIsCompanyOpen(contactConfig.officeHours as OfficeHour[]));
    };
    checkOpen();
      ...
      if (!mapRef.current) {
        ...
        if (!isNaN(lat) && !isNaN(lng)) {
          ...
          mapCoordinates.forEach((coord, idx) => {
            ...
            if (!isNaN(cLat) && !isNaN(cLng) && mapRef.current) {
  ...
  const displayName = shortName || name || 'CEC Nepal';
                ...
                {displayName && (
                  <h3>
                    ...
                    {displayName.split(' ').slice(1).join(' ') && (
                      <span>{displayName.split(' ').slice(1).join(' ')}</span>
                    )}
                  </h3>
                )}
              </div>
      ...
      <div className={styles.footerBottom}>
        <div className={styles.container}>
          <div className={styles.footerBottomContent}>
            <p>&copy; {new Date().getFullYear()} {(displayName || 'CEC Nepal').split(' ').join('')}. All Rights Reserved.</p>
